...
def chunkize_media(self, friendly_token, profiles, force=True):
    ...
    cmd = [
        ...
        "-c", # copy, not re-encoded
        "copy",
        "-f", # force container format
        "segment",
        "-segment_time",
...
def encode_media(
    ...
    if profile.extension == "gif":
        ...
        command = [
            ...
            "-ss", # seeks input file to position
            "3",
            "-i", # input file url
            media.media_file.path,
            "-hide_banner", # suppress printing of ffmpeg banner
            "-vf", # set the number of video frames to output
            "scale=344:-1:flags=lanczos,fps=1", 
            "-t", # When used as an output option (before an output url), 
